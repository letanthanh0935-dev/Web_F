...
*Ghi chú: Cần trao đổi thường xuyên để không bị trùng hoặc sót phần nào so với `requirements.md`.
...
│
├── images/
│   └── imgages/                  (ảnh từng sản phẩm, đặt tên trùng id trong products.json)
│
├── docs/
│   ├── project-plan.md            (kế hoạch)
│   └── requirements.md       (Chức năng từng trang)
│   └──dinh-huong-dat-hang.md  (định hướng cho chức năng đặt hàng)
└── README.md
```
